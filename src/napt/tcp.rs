...
use super::{calc_ip_checksum, calc_tcp_checksum, NAPTer};

impl NAPTer {
  pub fn translate_incoming_tcp(&self, mut ip_packet: MutableIpv4Packet) -> Option<MutableIpv4Packet<'static>> {
    ...
    if let Some(local_dest) = self.tcp_map.get(&u16::from_be(new_tcp_packet.get_destination())) {
      new_tcp_packet.set_checksum(calc_tcp_checksum(&mut ip_packet));
      new_ip_packet.set_checksum(calc_ip_checksum(&mut ip_packet));
  ...
  pub fn translate_outgoing_tcp(&mut self, ip_packet: MutableIpv4Packet) -> Option<MutableIpv4Packet<'static>> {
    ...
    new_tcp_packet.set_checksum(calc_tcp_checksum(&new_ip_packet));

use pnet::packet::{ipv4::MutableIpv4Packet, udp::MutableUdpPacket, Packet};

use super::{calc_ip_checksum, calc_udp_checksum, NAPTer};
...
impl NAPTer {
  pub fn translate_incoming_udp(&self, ip_packet: MutableIpv4Packet) -> Option<MutableIpv4Packet<'static>> {
    ...
    if let Some(local_dest) = self.tcp_map.get(&new_udp_packet.get_destination()) {
      ...
      new_udp_packet.set_checksum(calc_udp_checksum(&new_ip_packet));
      new_ip_packet.set_payload(new_udp_packet.packet());

...
use super::{calc_translated_ip_checksum, calc_translated_transport_checksum, NAPTer};
...
impl NAPTer {
  pub fn translate_incoming_tcp(&self, ip_packet: MutableIpv4Packet) -> Option<MutableIpv4Packet<'static>> {
    ...
    if let Some(local_dest) = self.tcp_map.get(&u16::from_be(new_tcp_packet.get_destination())) {
      new_ip_packet.set_checksum(
        calc_translated_ip_checksum(
        u16::from_be(new_ip_packet.get_checksum()),
        &new_ip_packet.get_destination(),
        &local_dest.0
      ).to_be());
      new_tcp_packet.set_checksum(
        calc_translated_transport_checksum(
        u16::from_be(new_tcp_packet.get_checksum()),
        &new_ip_packet.get_destination(),
        &local_dest.0,
        new_tcp_packet.get_destination(),
        local_dest.1
      ).to_be());
